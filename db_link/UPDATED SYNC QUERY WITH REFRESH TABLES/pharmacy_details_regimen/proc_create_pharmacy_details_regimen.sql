-- PROCEDURE: expanded_radet.proc_create_pharmacy_details_regimen()

-- DROP PROCEDURE IF EXISTS expanded_radet.proc_create_pharmacy_details_regimen();
...
create table expanded_radet.pharmacy_details_regimen AS
select * from dblink('db_link_ods',
'SELECT DISTINCT ON (p.person_uuid,p.ods_datim_id) p.person_uuid AS person_uuid40,p.ods_datim_id as pharma_ods_datim_id,
p.uuid, COALESCE(ds_model.display, p.dsd_model_type) as dsdModel, 
...
')AS sm(person_uuid40 character varying, pharma_ods_datim_id character varying,uuid character varying,
...
ADD CONSTRAINT unq_uuid_datimid_pharmacy_details_regimen UNIQUE (person_uuid40,pharma_ods_datim_id,uuid);
...
END 
$BODY$;
ALTER PROCEDURE expanded_radet.proc_create_pharmacy_details_regimen()
