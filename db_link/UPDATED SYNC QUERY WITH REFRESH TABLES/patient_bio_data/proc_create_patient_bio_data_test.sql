-- PROCEDURE: expanded_radet.proc_create_patient_bio_data_test()

-- DROP PROCEDURE IF EXISTS expanded_radet.proc_create_patient_bio_data_test();

CREATE OR REPLACE PROCEDURE expanded_radet.proc_create_patient_bio_data_test(
	)
LANGUAGE 'plpgsql'
AS $BODY$
...
EXECUTE format(
	'SELECT *
	 FROM dblink(''db_link_ods'',
				 ''SELECT count(uuid) 
				   FROM (SELECT uuid,ods_load_time
...
DROP TABLE IF EXISTS expanded_radet.patient_bio_data_test;

SELECT TIMEOFDAY() INTO start_time;

EXECUTE format('CREATE TABLE expanded_radet.patient_bio_data_test AS
...
ALTER TABLE expanded_radet.patient_bio_data_test
ADD CONSTRAINT unq_patient_bio_data UNIQUE (personuuid, bio_ods_datim_id);

CREATE INDEX idx_patient_bio_data_non_nulls ON expanded_radet.patient_bio_data_test (visit_date)
...
DELETE FROM expanded_radet.patient_bio_data_test
...
END 
$BODY$;
ALTER PROCEDURE expanded_radet.proc_create_patient_bio_data_test()
    OWNER TO lamisplus_etl;

--call expanded_radet.proc_create_patient_bio_data_test()