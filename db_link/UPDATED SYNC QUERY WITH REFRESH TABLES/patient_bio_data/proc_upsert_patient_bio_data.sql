...
'SELECT count(uuid) FROM ods_patient_person
					 WHERE ods_load_time > (select MAX(end_time) FROM streaming_remote_monitoring
										WHERE table_name = ''patient_bio_data'')')
AS sm(count bigint) INTO record_count;

INSERT INTO expanded_radet.patient_bio_data
SELECT * FROM dblink('db_link_ods',
...
PERFORM dblink('db_link_ods',
      format('INSERT INTO public.streaming_remote_monitoring(table_name,record_count,start_time,end_time) 
			 VALUES (''%s'',%L, %L, %L)',
             'patient_bio_data', record_count,start_time, end_time));
