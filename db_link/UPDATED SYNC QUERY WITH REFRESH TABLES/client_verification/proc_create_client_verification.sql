...
create table expanded_radet.client_verification AS
select * from dblink('db_link_ods',
...
')AS sm(client_person_uuid character varying,client_ods_datim_id character varying,
    uuid character varying,archived integer,clientVerificationOutCome text,clientVerificationStatus text,
    dateOfOutcome date);

ALTER TABLE expanded_radet.client_verification
ADD CONSTRAINT unq_uuid_datimid_client_verification UNIQUE (uuid, client_ods_datim_id);
