...
with DAG("lamisplus_maternal_cohort_datamart_server", start_date=datetime.datetime(2024, 7, 15), schedule_interval=None,
 default_args=default_args, catchup=True, max_active_runs=1) as dag:
    ...
    cte_maternal_cohort_joined = PostgresOperator(
        task_id="cte_maternal_cohort_joined",
        postgres_conn_id="pmtct_conn",
        sql = 'CALL maternal_cohort.proc_pmtct_maternal_cohort_joined()',
        autocommit = True
    )
    
    maternal_cohort = PostgresOperator(
        task_id="cte_maternal_cohort",
        postgres_conn_id="pmtct_conn",
    ...
    start >> upstream_tasks >> cte_maternal_cohort_joined >> maternal_cohort >> end
