...
def truncate_table(conn, table_name, periodcode):
    """
    Truncates a single familypartnerindex table.
    Accepts an active connection passed by the caller.
    """
    try:
        with conn.cursor() as cur:
            cur.execute(f"TRUNCATE familypartnerindex.{table_name}")
            conn.commit()
...
def run_single_procedure(procedure, datim):
    ...
    try:
        with connect_to_db.connect('lamisplus_ods_dwh')[0] as conn:
            ...
            with conn.cursor() as cur:
                cur.execute(f"CALL familypartnerindex.{procedure}(%s)", (datim,))
...
def run_proc_familypartnerindex_joined(datim):
    """
    Executes proc_familypartnerindex_joined for a datim_id.
    Opens its own short-lived connection — safe for concurrent use.
    """
    try:
        with connect_to_db.connect('lamisplus_ods_dwh')[0] as conn:
            conn.autocommit = True
            with conn.cursor() as cur:
                cur.execute("CALL familypartnerindex.proc_familypartnerindex_joined(%s)", (datim,))
        logger.info(f"Successfully executed familypartnerindex_joined for {datim}")
    except Exception as e:
        logger.error(f"Error occurred executing familypartnerindex_joined for {datim}: {e}")
...
def generate_cte_concurrently(datim_ids, procedures, max_workers):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        executor.map(run_proc_familypartnerindex_joined, datim_ids)


def run_final_familypartnerindex(conn, ip_name):
    """
    Executes the final familypartnerindex procedure for an IP.
    Accepts an active connection passed by the caller.
    """
    try:
        with conn.cursor() as cur:
            cur.execute("CALL familypartnerindex.proc_final_familypartnerindex(%s)", (ip_name,))
            conn.commit()
            logger.info(f"Successfully executed final_familypartnerindex for {ip_name}")
    except Exception as e:
        logger.error(f"Error occurred executing final_familypartnerindex for {ip_name}: {e}")

def run_final_familypartnerindex_for_ips(ip_names, periodcode):
    def _run(ip_name):
        with connect_to_db.connect('lamisplus_ods_dwh')[0] as conn:
            run_final_familypartnerindex(conn, ip_name)
    
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        executor.map(_run, ip_names)
    logger.info(f"Batch of {len(ip_names)} final_familypartnerindex procedures executed for {periodcode}")

def generate_familypartnerindex_report(**kwargs):
    """
    ...
    table_names = [
        "familypartnerindex_joined", "familypartnerindex_monitoring",
        "family_index", "partner_index"
    ]

    procedures = [
        "proc_family_index", "proc_partner_index"
    ]
        ...
        with connect_to_db.connect('lamisplus_ods_dwh')[0] as conn:
            ...
            for ip in ip_names:
                ...
                if datim_ids:
                    ...
                    generate_cte_concurrently(datim_ids, procedures, max_workers=30)

        # 3. Final rollup — sequential, shared connection
        run_final_familypartnerindex_for_ips(ip_names, periodcode)


if __name__ == '__main__':
    generate_familypartnerindex_report()
