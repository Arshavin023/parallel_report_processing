...
def truncate_table(conn, table_name, periodcode):
    """
    Truncates a single pmtcthts table.
    Accepts an active connection passed by the caller.
    """
    try:
        with conn.cursor() as cur:
            ...
            conn.commit()
            logger.info(f"Table {table_name} truncated successfully {periodcode}.")
    except Exception as e:
...
def run_single_procedure(procedure, datim):
    ...
    try:
        with connect_to_db.connect('lamisplus_ods_dwh')[0] as conn:
            ...
            with conn.cursor() as cur:
                cur.execute(f"CALL pmtcthts.{procedure}(%s)", (datim,))
...
def run_proc_pmtcthts_joined(datim):
    """
    Executes proc_pmtcthts_joined for a datim_id.
    Opens its own short-lived connection — safe for concurrent use.
    """
    try:
        with connect_to_db.connect('lamisplus_ods_dwh')[0] as conn:
...
def run_final_pmtcthts(conn, ip_name, period):
    """
    Executes the final pmtcthts procedure for an IP.
    Accepts an active connection passed by the caller.
    """
    try:
        with conn.cursor() as cur:
            cur.execute("CALL pmtcthts.proc_final_pmtcthts(%s)", (ip_name,))
            conn.commit()
...
def run_final_pmtcthts_for_ips(ip_names, periodcode):
    def _run(ip_name):
        with connect_to_db.connect('lamisplus_ods_dwh')[0] as conn:
            run_final_pmtcthts(conn, ip_name, periodcode)
    
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        executor.map(_run, ip_names)
    logger.info(f"Batch of {len(ip_names)} final_pmtcthts procedures executed for {periodcode}")
...
def generate_pmtcthts_report(**kwargs):
    ...
    for periodcode in periods:
        ...
        # 4. Final rollup 
        run_final_pmtcthts_for_ips(ip_names, periodcode)
