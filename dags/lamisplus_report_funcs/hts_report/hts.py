...
def run_hts_mapping(datim):
    try:
        with connect_to_db.connect('lamisplus_ods_dwh')[0] as conn:
            with conn.cursor() as cur:
                cur.execute(f"CALL hts.proc_hts_mapping('{datim}')")
                conn.commit()
                logger.info(f"Procedure proc_hts_mapping for {datim} executed successfully.")
    except psycopg2.OperationalError as e:
        logger.error(f"Operational error occurred while processing {datim} for procedure: {e}")

def run_final_hts(ip_name,periodcode:str):
    try:
        with connect_to_db.connect('lamisplus_ods_dwh')[0] as conn:
            with conn.cursor() as cur:
                cur.execute(f"CALL hts.proc_final_hts('{ip_name}')")
                conn.commit()
