...
def run_single_procedure(procedure, datim):
    ...
    try:
        with connect_to_db.connect('lamisplus_ods_dwh')[0] as conn:
            ...
            with conn.cursor() as cur:
                cur.execute(f"CALL prep.{procedure}(%s)", (datim,))
...
def run_proc_prep_joined_insert(datim):
    """
    Executes proc_prep_joined for a datim_id.
    Opens its own short-lived connection — safe for concurrent use.
    """
    try:
        with connect_to_db.connect('lamisplus_ods_dwh')[0] as conn:
...
def run_final_prep(conn, ip_name, periodcode):
    """
    Executes the final PrEP procedure for an IP.
    Accepts an active connection passed by the caller.
    """
    try:
        with conn.cursor() as cur:
            cur.execute("CALL prep.proc_final_prep(%s)", (ip_name,))
            conn.commit()
...
def run_final_prep_for_ips(ip_names, periodcode):
    def _run(ip_name):
        with connect_to_db.connect('lamisplus_ods_dwh')[0] as conn:
            run_final_prep(conn, ip_name, periodcode)
    
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        executor.map(_run, ip_names)
    logger.info(f"Batch of {len(ip_names)} final_prep procedures executed for {periodcode}")
...
def generate_prep_report(**kwargs):
    ...
    table_names = [
        "prep.prep_joined", "prep.cte_baselineclinic",
        "prep.cte_current_pc",
