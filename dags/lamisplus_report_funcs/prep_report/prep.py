...
def run_single_procedure(datim):
    try:
        with connect_to_db.connect('lamisplus_ods_dwh')[0] as conn:
            with conn.cursor() as cur:
                cur.execute(f"CALL prep.proc_prep_joined('{datim}')")
                conn.commit()
                logger.info(f"Procedure proc_prep_joined for {datim} executed successfully.")
    except psycopg2.OperationalError as e:
...
def generate_prep_report(**kwargs):
    ...
    table_names = ["prep.prep_joined","prep.prep_monitoring"]
