...
def truncate_table(conn, table_name,periodcode):
    """
    Truncates a single maternalcohort table.
    Accepts an active connection passed by the caller.
    """
    try:
        with conn.cursor() as cur:
...
def run_single_procedure(procedure, datim):
    ...
    try:
        with connect_to_db.connect('lamisplus_ods_dwh')[0] as conn:
            ...
            with conn.cursor() as cur:
                cur.execute(f"CALL maternalcohort.{procedure}(%s)", (datim,))
...
def run_proc_maternalcohort_joined(datim):
    """
    Executes proc_maternalcohort_joined for a datim_id.
    Opens its own short-lived connection — safe for concurrent use.
    """
    try:
        with connect_to_db.connect('lamisplus_ods_dwh')[0] as conn:
...
def run_final_maternalcohort(conn, ip_name, period):
    """
    Executes the final maternalcohort procedure for an IP.
    Accepts an active connection passed by the caller.
    """
    try:
        with conn.cursor() as cur:
            cur.execute("CALL maternalcohort.proc_final_maternalcohort(%s)", (ip_name,))
            conn.commit()
...
def run_final_maternalcohort_for_ips(ip_names, periodcode):
    def _run(ip_name):
        with connect_to_db.connect('lamisplus_ods_dwh')[0] as conn:
            run_final_maternalcohort(conn, ip_name, periodcode)
    
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        executor.map(_run, ip_names)
    logger.info(f"Batch of {len(ip_names)} final_maternalcohort procedures executed for {periodcode}")
...
def generate_maternalcohort_report(**kwargs):
    ...
    for periodcode in periods:
        ...
        run_final_maternalcohort_for_ips(ip_names, periodcode)
