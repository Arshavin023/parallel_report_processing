...
def fetch_datim_ids(ip_name):
    fetch_datims_query = "SELECT datim_id FROM central_partner_mapping WHERE ip_name=%s"
...
def run_procedures_for_datim(datim):
    procedures = ["proc_bio_data","proc_biometric","proc_carecardcd4","proc_case_manager",
            "proc_cervical_cancer","proc_client_verification","proc_crytococal_antigen",
            "proc_current_clinical","proc_current_regimen","proc_current_status",
            "proc_current_tb_result","proc_current_vl_result","proc_dsd1","proc_dsd2",
            "proc_eac","proc_ipt","proc_ipt_s","proc_iptnew","proc_labcd4",
            "proc_naive_vl_data","proc_ovc","proc_patient_lga","proc_pharmacy_details_regimen",
            "proc_sample_collection_date","proc_tb_sample_collection","proc_tblam",
            "proc_tbstatus","proc_tbtreatment","proc_tbtreatmentnew","proc_vacauseofdeath"]
...
# Function to run only `proc_radet_joined_insert` for a single datim_id
def run_proc_radet_joined_insert(datim):
    try:
        cur2.execute(f"CALL expanded_radet_client.proc_radet_joined_insert('{datim}')")
        dwh_conn.commit()
    except Exception as e:
...
if __name__ == '__main__':
    datim_ids=fetch_datim_ids('HAN-KP-CARE 1')
