...
credential_path = r'C:\Users\5300\Documents\Palladium\database_credentials\config.ini'
csv_file_path = r'C:\Users\5300\Documents\Palladium\lamisplus_sync_ods_pipeline\dags\files\datatypes.csv'
python_datetime = 'datetime64[ns]'
constraint_type_one = 'uuid, ods_datim_id'
constraint_type_two = 'id, ods_datim_id'
constraint_type_three = 'id, uuid, ods_datim_id'
# '/home/lamisplus/database_credentials/config.ini'

def read_db_config(filename=credential_path, section='database'):
    # Create a parser
...
dtype_mapping = {
    'bigint': 'int64',
    'integer': 'int64',
    'timestamp without time zone': python_datetime,
    'date': python_datetime,
    'boolean': 'bool',
    'double precision': 'float64',
    'timestamp with time zone': python_datetime,
    'smallint': 'int64',
    'bytea': 'object',
    'text': 'str',
    'uuid': 'uuid.UUID'
...
def apply_dtype_transforms(df, dtype_cols):
    """
    Apply transformations to columns based on their data types.
    """
    for col_name, col_dtype in dtype_cols.items():
        if col_name in df.columns:
            df[col_name] = df[col_name].apply(lambda x: dict(x).get('value') if x != None else None)
            if col_dtype == python_datetime:
                df[col_name] = pd.to_datetime(df[col_name], errors='coerce')
            elif col_dtype in ['int64', 'float64']:
                df[col_name] = pd.to_numeric(df[col_name], errors='coerce')
            elif col_dtype == 'bool':
                df[col_name] = df[col_name].astype('bool', errors='raise')
    return df

def transform_ods_df(df, table_name, dtype=None):
    if dtype is not None:
        # Apply transformations for the provided dtype
        dtype_cols = {col: dtype_mapping[dtype] for col, dtype in dtype.items()}
        df = apply_dtype_transforms(df, dtype_cols)

    # Replace empty strings with NaN
    df = df.replace(r'^\s*$', np.nan, regex=True)

    # Fix data types based on the CSV data type table
    df_cols = df.columns
    df_data_types = pd.read_csv(csv_file_path)
    df_dtype_table = df_data_types[df_data_types['table_name'] == table_name][['column_name', 'data_type']]
    
    # Only consider relevant columns
    arr_dtype_cols = df_dtype_table[~df_dtype_table['data_type'].isin(['jsonb', 'character varying'])]
    
    # Create a mapping of column names to their corresponding types
    dict_dtypes = {col_name: dtype_mapping[col_dtype] for col_name, col_dtype in arr_dtype_cols.values if col_name in df_cols}

    # Process the columns by type
    df = apply_dtype_transforms(df, dict_dtypes)
...
def process_stg_to_ods(table_name, constraints, dtype=None):
    ...
    cur.execute("""SELECT file_name 
                ...
                LIMIT 10  --(SELECT (COUNT(file_name)/2) 
                        --FROM stg_monitoring WHERE processed='N')
                """, (staging_table,))
               
    file_names = cur.fetchall()
    file_names = tuple([x[0] for x in file_names])
    # print(file_names)
    load_time = datetime.datetime.now()
    
    # Unzip the list of tuples into three separate lists (one for each element)
    if file_names:
        try:
            print(f'Processing {table_name} data...')

            df = pd.read_sql(f"SELECT * FROM {staging_table} WHERE stg_file_name IN %s ORDER BY stg_load_time ASC", con=staging_conn, params=(file_names,))
            df = df.drop(['stg_batch_id', 'stg_load_time', 'stg_file_name'], axis=1)
            stg_constraints = constraints.replace('ods_datim_id','stg_datim_id')
            stg_constraints = [col.strip() for col in stg_constraints.split(',')]
            # ", ".join([f"'{item.strip()}'" for item in stg_constraints.split(',')])
            print(stg_constraints)
            df.drop_duplicates(subset=stg_constraints,keep='last', inplace=True)
            ...
            if not df.empty:
                ...
                try:
                    ...
                    print(f'Storing ods data on {staging_table}...')
                    store_ods_df(df_transformed, table_name, constraints, dtype=dtype)
                    print(f'number of records pushed to ods: {record_count}')
                    print(f'Successfully stored ods data for {ods_table}...')
                    # Use connection for updates
                    update_query_success = """
                    UPDATE stg_monitoring 
                    SET processed='Y', stg_deleted='N', error_message='No errors'
                    WHERE table_name = %s AND file_name IN %s
                    """
                    cur.execute(update_query_success, (staging_table,file_names))
                    staging_conn.commit()
                    ...
                    WHERE table_name = %s AND file_name IN %s
                    """
                    cur.execute(update_query_failed,(error_message, staging_table, file_names))
                    staging_conn.commit()
            ...
            print(f"Error unpacking ls_to_process: {e}. ls_to_process might be empty or malformed.")
