...
with DAG("lamisplus_radet_generation_datamart_server",start_date=datetime.datetime(2024, 7, 1),schedule_interval=None,
    ...
    update_central_parner_mapping_table = PostgresOperator(
        task_id="update_central_parner_mapping_table",
        postgres_conn_id="radet_conn",
        sql='call expanded_radet.proc_update_public_central_partner_mapping()',
        autocommit=True
        )
    ...
    with TaskGroup(group_id='upstream_tasks') as upstream_tasks:
        ...
        current_eac = PostgresOperator(
            task_id="current_eac",
            postgres_conn_id="radet_conn",
            sql='call expanded_radet.proc_current_eac()',
            autocommit=True
        )
        
        first_hes = PostgresOperator(
            task_id="first_hes",
            postgres_conn_id="radet_conn",
            sql='call expanded_radet.proc_first_hes()',
            autocommit=True
        )
        
        last_hes = PostgresOperator(
            task_id="last_hes",
            postgres_conn_id="radet_conn",
            sql='call expanded_radet.proc_last_hes()',
            autocommit=True
        )
        
        extended_hes = PostgresOperator(
            task_id="extended_hes",
            postgres_conn_id="radet_conn",
            sql='call expanded_radet.proc_extended_hes()',
            autocommit=True
        )
        
        post_eac_vl = PostgresOperator(
            task_id="post_eac_vl",
            postgres_conn_id="radet_conn",
            sql='call expanded_radet.proc_post_eac_vl()',
            autocommit=True
        )
    ...
    with TaskGroup(group_id='midstream_tasks') as midstream_tasks:
        ...
        first_eac = PostgresOperator(
            task_id="first_eac",
            postgres_conn_id="radet_conn",
            sql='call expanded_radet.proc_first_eac()',
            autocommit=True
        )
        
        last_eac = PostgresOperator(
            task_id="last_eac",
            postgres_conn_id="radet_conn",
            sql='call expanded_radet.proc_last_eac()',
            autocommit=True
        )
        
        extended_eac = PostgresOperator(
            task_id="extended_eac",
            postgres_conn_id="radet_conn",
            sql='call expanded_radet.proc_extended_eac()',
            autocommit=True
        )
        
        eac_count = PostgresOperator(
            task_id="eac_count",
            postgres_conn_id="radet_conn",
            sql='call expanded_radet.proc_eac_count()',
            autocommit=True
        )
    ...
    start >> update_central_parner_mapping_table >> update_period_table >> upstream_tasks >> midstream_tasks >> downstream_tasks >> radet_joined >> expanded_radet_weekly >> end
