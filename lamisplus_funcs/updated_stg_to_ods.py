...
# Define your SQL query to read in data_type from information_schema.columns
data_type_query = """
...
def process_stg_to_ods(table_name, constraints, dtype=None):
    ...
    for datim_id, batch_id, file_name in ls_to_process:  
        df = pd.read_sql("select * from {} where stg_datim_id = '{}' and stg_batch_id = '{}' and stg_file_name = '{}'".format(staging_table, datim_id, batch_id, file_name), con=stg_engine)
        ...
        if not df.empty:
            ...
            try:
                ...
                stg_engine.execute(f"update stg_monitoring set processed='F',error_message= '{e}' where table_name = '{staging_table}' and datim_id = '{datim_id}' and batch_id = '{batch_id}' and file_name = '{file_name}'")
                pass
    ...
    try:
        ...
        insert_query = """
        INSERT INTO stg_to_ods_monitoring (table_name, record_count, start_time, end_time) 
        VALUES (%s, %s, %s, %s)
        """
        params = (ods_table, record_count, load_time, end_time)
        dwh_engine.execute(insert_query, params)
        print(f'{record_count} staging records successfully moved to {ods_table} on data warehouse and logs inserted into stg_to_ods_monitoring table')
